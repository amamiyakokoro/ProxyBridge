...
RuleAction match_rule_inner(const char *process_name, UINT32 dest_ip, UINT16 dest_port, BOOL is_udp, UINT32 *out_proxy_config_id)
{
    ...
    while (rule != NULL)
    {
        ...
        // DNS leak protection is protocol-independent: when enabled, any PROXY rule
        // carries both UDP/53 and TCP/53. When disabled, plaintext DNS skips PROXY
        // rules even if the rule otherwise covers that transport.
        BOOL is_proxy_dns = dest_port == 53 && rule->action == RULE_ACTION_PROXY;
        if (is_proxy_dns && !g_dns_hijack_enabled)
        {
            rule = rule->next;
            continue;
        }
        ...
        if (!is_proxy_dns && rule->protocol != RULE_PROTOCOL_BOTH)
        {
